...
from utils_noise import MovingAverage, get_corrected_label
...
def training(args):
    ...
    scheduler_img = CosineAnnealingLR(optimizer_img, T_max=args.epochs, eta_min=1e-7)
    scheduler_pt = CosineAnnealingLR(optimizer_pt, T_max=args.epochs, eta_min=1e-7)
    print(f"Using Cosine Annealing LR: {args.lr_img} -> 1e-7 over {args.epochs} epochs")
    ...
    # Initialize Moving Average (after dataset is loaded)
    if args.use_ma:
        print("Initializing Moving Average...")
        MA_img = MovingAverage(num_data=len(unlabeled_trainset), num_classes=args.num_classes, beta=args.ma_beta, warm_up=args.ma_warmup)
        MA_pt = MovingAverage(num_data=len(unlabeled_trainset), num_classes=args.num_classes, beta=args.ma_beta, warm_up=args.ma_warmup)
        # Initialize bank
        MA_img.initial_bank()
        MA_pt.initial_bank()
    ...
    for epoch in range(args.epochs):
        if epoch == 0:
            ...
            if args.dataset == '3DMNIST':
                ...
                for data in val_loader:
                    img_feat, pt_feat, ori_label, indices = data
                    with torch.no_grad():
                ...
                val_loader = torch.utils.data.DataLoader(labeled_trainset, batch_size=16, shuffle=False, num_workers=10)
                for data in val_loader:
                    pt, img1, img2, target, target_vec, indices = data
                    with torch.no_grad():
        ...
        # === Cosine Annealing会自动调整学习率 ===
        # 在epoch结束时调用scheduler.step()即可
        ...
        for idx in range(niters_per_epoch):
            ...
            if args.dataset == '3DMNIST': 
                ...
                unsup_pts = unsup_pts.cuda(non_blocking=True)
                unsup_indices = unsup_minibatch[5].cuda(non_blocking=True)
            
            if args.dataset == '3DMNIST':
                unsup_indices = unsup_minibatch[3].cuda(non_blocking=True)
            ...
            # MA只负责平滑，不做锐化
            # 锐化统一在get_corrected_label中进行
            if args.use_ma and epoch >= args.ma_warmup:
                # Image MA
                soft_img_1 = torch.softmax(logits_img_tea_1, dim=1)
                soft_img_2 = torch.softmax(logits_img_tea_2, dim=1)
                smooth_img_1 = MA_img.update(soft_img_1, unsup_indices, epoch)
                smooth_img_2 = MA_img.update(soft_img_2, unsup_indices, epoch)
                logits_img_tea_1 = torch.log(smooth_img_1 + 1e-8)
                logits_img_tea_2 = torch.log(smooth_img_2 + 1e-8)
                
                # Point Cloud MA
                soft_pt_1 = torch.softmax(logits_pt_tea_1, dim=1)
                soft_pt_2 = torch.softmax(logits_pt_tea_2, dim=1)
                smooth_pt_1 = MA_pt.update(soft_pt_1, unsup_indices, epoch)
                smooth_pt_2 = MA_pt.update(soft_pt_2, unsup_indices, epoch)
                logits_pt_tea_1 = torch.log(smooth_pt_1 + 1e-8)
                logits_pt_tea_2 = torch.log(smooth_pt_2 + 1e-8)
            ...
            # === KL Consistency Loss ===
            # Teacher-Student一致性，保留细粒度信息
            log_prob_img_stu = torch.nn.functional.log_softmax(logits_img_stu_1, dim=1)
            prob_img_tea = torch.softmax(logits_img_tea_1.detach(), dim=1)
            loss_kl_img = kl_loss_func(log_prob_img_stu, prob_img_tea)
            
            log_prob_pt_stu = torch.nn.functional.log_softmax(logits_pt_stu_1, dim=1)
            prob_pt_tea = torch.softmax(logits_pt_tea_1.detach(), dim=1)
            loss_kl_pt = kl_loss_func(log_prob_pt_stu, prob_pt_tea)
            
            loss_kl = (loss_kl_img + loss_kl_pt) / 2
            ...
            if args.use_low_confidence_correction:
                # 1. 识别低置信度样本 (mask_img1是高置信度mask，取反得到低置信度mask)
                mask_img1_low = 1.0 - mask_img1
                mask_img2_low = 1.0 - mask_img2
                mask_pt1_low = 1.0 - mask_pt1
                mask_pt2_low = 1.0 - mask_pt2
                
                # 2. 获取修正后的标签 (利用多模态一致性 + 锐化)
                # 锐化统一在这里进行，避免重复锐化
                temp = args.sharpen_temp if (args.use_sharpening and epoch >= args.sharpen_start) else 1.0
                corrected_img1 = get_corrected_label(logits_img_tea_1, logits_pt_tea_1, temperature=temp)
                corrected_img2 = get_corrected_label(logits_img_tea_2, logits_pt_tea_2, temperature=temp)
                corrected_pt1 = get_corrected_label(logits_pt_tea_1, logits_img_tea_1, temperature=temp)
                corrected_pt2 = get_corrected_label(logits_pt_tea_2, logits_img_tea_2, temperature=temp)
                
                # 3. 计算修正损失 (软标签交叉熵)
                loss_img1_low = -torch.sum(torch.nn.functional.log_softmax(logits_img_stu_1, dim=1) * corrected_img1, dim=1)
                loss_img2_low = -torch.sum(torch.nn.functional.log_softmax(logits_img_stu_2, dim=1) * corrected_img2, dim=1)
                loss_pt1_low = -torch.sum(torch.nn.functional.log_softmax(logits_pt_stu_1, dim=1) * corrected_pt1, dim=1)
                loss_pt2_low = -torch.sum(torch.nn.functional.log_softmax(logits_pt_stu_2, dim=1) * corrected_pt2, dim=1)
                
                # 4. 只对低置信度样本计算平均损失
                correction_loss = (
                    (loss_img1_low * mask_img1_low).mean() +
                    (loss_img2_low * mask_img2_low).mean() +
                    (loss_pt1_low * mask_pt1_low).mean() +
                    (loss_pt2_low * mask_pt2_low).mean()
                ) / 4
                
                # 5. 加到总损失中 (带权重)
                loss = loss + args.weight_correction * correction_loss
            else:
                correction_loss = torch.tensor(0.0).cuda()
            ...
            # === 旧的iteration-based LR scheduler已删除 ===
            # 之前这里有bug代码导致学习率过早下降
            # 现在统一使用epoch-based scheduler (在epoch循环开始处)
            ...
            if iteration % args.per_print == 0:
                ...
                print("loss: %f  crc: %f align: %f cps: %f kl: %f corr: %f quan: %f semi_mg: %f" % 
                      (loss.item(), crc_loss.item(), align_loss.item(), cps_loss.item(), 
                       loss_kl.item(), correction_loss.item(), quan_loss.item(), local_semi_mg_loss.item()))
            ...
            if  (iteration%niters_per_epoch) == 0:
                # === Epoch结束：更新学习率 ===
                # 独立于保存/验证逻辑，确保每个epoch都会更新LR
                scheduler_img.step()
                scheduler_pt.step()
                current_lr_img = optimizer_img.param_groups[0]['lr']
                current_lr_pt = optimizer_pt.param_groups[0]['lr']
                print(f"\n=== Epoch {epoch} Complete ===")
                print(f"LR updated - Image: {current_lr_img:.6e}, Point: {current_lr_pt:.6e}\n")
...
if __name__ == "__main__":
    ...
    #optimizer
    parser.add_argument('--lr_img', type=float, default=1e-4, metavar='LR',  help='learning rate (default: 0.001, 0.1 if using sgd)')
    
    parser.add_argument('--lr_pt', type=float, default=2e-4, metavar='LR',
                        help='learning rate (default: 0.001, 0.1 if using sgd)')
    
    parser.add_argument('--lr_step', type=int,  default=20,
                        help='[DEPRECATED] Not used with Cosine Annealing LR')
    
    parser.add_argument('--weight_decay', type=float, default=1e-5, metavar='weight_decay',
                        help='learning rate (default: 1e-3)')

    # MA and Correction arguments
    parser.add_argument('--use_ma', action='store_true', help='use moving average for teacher predictions')
    parser.add_argument('--ma_beta', type=float, default=0.9, help='MA decay rate')
    parser.add_argument('--ma_warmup', type=int, default=3, help='MA warm-up epochs')
    parser.add_argument('--use_low_confidence_correction', action='store_true', help='use correction loss for low confidence samples')
    parser.add_argument('--weight_correction', type=float, default=1.0, help='weight for correction loss')
    
    # Sharpening arguments
    parser.add_argument('--use_sharpening', action='store_true', help='use sharpening for pseudo labels')
    parser.add_argument('--sharpen_temp', type=float, default=0.5, help='sharpening temperature (0.3-1.0, lower=sharper)')
    parser.add_argument('--sharpen_start', type=int, default=5, help='start sharpening after N epochs')
